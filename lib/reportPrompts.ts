...
14. 한자(漢字)는 한국어 리포트에서도 쓰지 않는다 — "화(火)", "대운(大運)"처럼 괄호로 병기하지 말고 한글만 쓴다. 원소는 이모지(🌳🔥⛰️💎💧)로 표시한다.
15. 수치를 말로 옮길 때 기준을 일관되게 쓴다: 오행은 30% 이상 "강하다/우세", 15~29% "보통", 14% 이하 "약하다/적다"이고 같은 값은 어느 페이지에서나 같은 말로 부른다. 심리검사 축은 데이터에 적힌 방향(높음/낮음)과 강도 표기와 모순되는 강도어를 쓰지 않는다.
16. strengths 항목은 정말 강점만 쓴다: "~가 적어서/약해서 ~하지 못한다" 같은 결핍·약점 서술로 시작하거나 채우지 말고, 그 데이터가 가져다주는 힘(버티는 힘, 민감하게 알아채는 감각 등)으로 풀어 쓴다.
17. (한국어일 때) 문체는 앱 화면과 같은 해요체("~예요", "~해요", "~이에요")로 처음부터 끝까지 통일한다. 위 문체 예시가 합쇼체("~입니다")인 것은 밀도와 장면 묘사 방식을 보여 주려는 것일 뿐이니 어미는 따라 하지 말 것. 독자는 항상 "${context.nickname}님"으로 부르고 "님"을 빼지 않는다. "~하지 않으신가요"보다 "~ 아니세요?", "가장 저장할 문장은 이겁니다"보다 "남기고 싶은 문장은 이거예요"처럼 자연스러운 입말로 쓴다.
18. 사주 용어는 이 언어의 용어집대로 쓰고(예: 대운은 영어 "10-year cycle", 스페인어 "ciclo de
