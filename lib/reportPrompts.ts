...
export function describeUpcomingPeriod(
  ...
  return `다가오는 대운 시기: ${ageRange}, ${elementLabel} 기운이 강해지는 시기 (현재 만 ${currentAge}세 기준 — 이 시기는 아직 오지 않았음, 실제 계산된 사주 데이터이므로 확정된 사실로 서술할 것. upcoming_period_preview_body, upcoming_period_body, closing_body에서 나이를 언급할 때 반드시 이 숫자 그대로 문장 앞쪽에서 명확히 쓸 것 — "머지않아"/"언젠가"/"아마"처럼 흐리지 말고, 임의로 바꾸거나 다른 나이대를 지어내지 말 것)`;
}
