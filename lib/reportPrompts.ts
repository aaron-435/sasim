...
export function describeUpcomingPeriod(
  ...
  return `다가오는 대운 시기: ${ageRange}, ${elementLabel} 기운이 강해지는 시기 (현재 만 ${currentAge}세 기준 — 이 시기는 아직 오지 않았음, 실제 계산된 사주 데이터이므로 확정된 사실로 서술할 것. upcoming_period_body와 closing_body에서 나이를 언급할 때 반드시 이 숫자 그대로 문장 앞쪽에서 명확히 쓸 것 — "머지않아"/"언젠가"/"아마"처럼 흐리지 말고, 임의로 바꾸거나 다른 나이대를 지어내지 말 것)`;
}
...
${caseFields}
  ...
  const paidFields = `
  "upcoming_period_heading": "'다가오는 시기' 섹션 소제목 — 아래 데이터의 '다가오는 대운 시기' 줄에 나온 나이와 원소를 제목 맨 앞에서 숫자 그대로 밝히고, 지금까지의 시기가 저물고 다음 장이 시작된다는 담담한 전환 프레임으로 쓴다. 예: '32세부터, 물의 계절이 열립니다' / '32세부터 시작되는 다음 장'. '머지않아'·'언젠가'처럼 나이를 흐리는 말로 시작하지 않는다. 그 줄이 '정보 없음'이거나 '범위를 벗어남'이면 나이 없이 '다가오는 흐름' 정도의 일반적인 제목",
  "upcoming_period_body": "3~4문장. 첫 문장부터 '다가오는 대운 시기' 데이터의 나이 숫자를 그대로 명확히 밝히며 시작한다('머지않아'·'언젠가'·'곧' 같은 흐린 시점 표현으로 시작하지 말 것). 지금까지의 시기가 저물고 새로운 국면(다음 장)이 시작된다는 확정된 사실로 서술하고, 그 기운 전환 이후 자연스럽게 따라오는 변화, 그리고 지금부터 무엇을 준비해 두면 좋은지를 담담하게 이어 쓴다. 나이 숫자는 그 데이터 줄에 있는 그대로만 쓰고 절대 새로 만들어내지 말 것 — 정보가 없다고 나오면 숫자 없이 일반적인 흐름으로만 쓰되, 정보가 없다는 사실 자체를 문장에 쓰지 말 것. 이미 지난 시기를 다루지 말고 반드시 앞으로 올 시기만 다룰 것.",
  ...
  "closing_body": "마무리 문단 3~4문장. 이 리포트는 이미 upcoming_period_body에서 같은 나이·기운 전환을 한 번 밝혔다는 걸 전제로 쓴다 — 그 문장을 표현만 바꿔 다시 설명하지 말고('전환은 분명해요'→'변화는 분명해요' 같은 동의어 교체 금지), 앞서 나온 이야기를 짧게 되짚듯 첫 문장(들)에서 나이와 원소를 숫자 그대로 밝히며 기운 전환을 확정된 사실로 쓰되('~일 거예요'처럼 흐리지 말고 '~됩니다/~해요'처럼 단정), 그 뒤로는 upcoming_period_body에 없던 새로운 각도(예: 지금 이 순간과의 대비, 구체적으로 달라지는 하루의 장면, 이 리포트 전체를 닫는 감정적 의미)에서 이어 쓴다. 이어서 이번 모듈(아래 데이터의 심리테스트 모듈)이 다루는 구체적 영역에 한정해서, 그 시기 이후 달라지는 결과도 확정 문장으로 쓴다 — 예: 번아웃 모듈이면 '회복됩니다'가 아니라 '무겁던 느낌이 가벼워져요' 같은 체감 표현, 분노 모듈이면 '치받는 느낌이 가라앉아요', 돈 모듈이면 '여유가 생겨요'처럼 이번 모듈 주제에서만 나올 수 있는 문장. 다른 모듈 리포트에 그대로 붙여도 말이 되는 보편적 약속('인생이 다 잘 풀립니다', '다 잘될 거예요')은 절대 쓰지 않는다 — 이게 가장 큰 실패 조건이다. 정신건강 계열 모듈(번아웃·수면·분노 등)에서는 '회복됩니다/완치됩니다'처럼 임상적으로 들리는 말 대신 몸으로 느껴지는 감각 표현을 쓴다. '다가오는 대운 시기' 데이터가 '정보 없음'이거나 '범위를 벗어남'이면 나이 숫자 없이 이번 모듈 영역에 한정된 확정 문장만 쓴다. 이 리포트 전체에서 가장 저장하고 싶은 문장으로 끝낼 것"`;
...
### 안전
8. 진단처럼 읽히는 단정, 겁주기(건강·사고·이별 예측)를 쓰지 않는다. 전문 심리상담·의료를 대체하지 않는 참고 자료의 톤을 유지한다. 단, 기운 전환(나이+원소)과 이번 모듈이 다루는 구체적 영역 안의 결과를 확정 문장으로 쓰는 것은 이 규칙 위반이 아니다 — 금지되는 것은 모듈 범위를 벗어난 보편적 약속과 건강·사고 등 부정적 단정이다.
