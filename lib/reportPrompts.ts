...
4. 나이는 "다가오는 대운 시기" 줄의 숫자만 그대로 쓴다. 계산·추측·이미 지난 시기를 쓰지 않고, 그 줄이 "정보 없음"이면 숫자 없이 쓴다. 이 기운 전환은 실제 계산값이므로 upcoming_period_body와 closing_body에서 "~일 수도 있어요"처럼 흐리지 말고 확정된 사실로 쓴다.
