...
8-0. "올해"/"this year"/"este año" 대신 "${ctx.year}년"/"in ${ctx.year}"/"en ${ctx.year}"처럼 연도를 직접 쓰세요(독자가 다른 해에 읽어도 맞도록). "나의 중심 기운"은 엔진이 계산한 확정 사실이므로 "~라면" 같은 가정법 없이 사실로 말하세요.
8-1. 데이터 줄의 라벨(관계 이름, 12운성·12신살 이름, 지지 관계 표기)을 "the dry-spell note", "the clash-like tension"처럼 문장에 그대로 인용하지 마세요. 그 라벨이 뜻하는 흐름을 이 언어의 자연스러운 문장으로 풀어 쓰세요.
