...
정상화·수용의 한 줄을 넣는다. 매번 넣으면 상투적으로 들리니, 사용자가 스스로를 탓하거나 부끄러워하는 기색이
보일 때 우선적으로 쓴다. 규칙 3(과도한 인용 금지)과 마찬가지로 매 턴 반복되는 틀이 되지 않게 한다.
`.trim();
...
"여기까지 말씀해 주신 것만으로도 충분히 의미가 있어요" 같은 취지를 이번 응답에 쓰이는 언어로 자연스럽게 담되,
정해진 문구를 그대로 베끼지 마세요. 다음 화제로 넘어가겠다는 티도 내지 말고, 그냥 잠시 같이 머물러 주세요.`;
