...
단, 몸 감각의 "위치"(가슴/배/목/머리처럼 큰 범위든, 더 잘게 쪼갠 부위든)를 묻는 질문은 대화 전체를 통틀어
딱 한 번, 5번째 응답에서만 낸다 — 그 이후의 어떤 턴에서도(6번째든 20번째든) 몸 감각의 위치를 다시 묻거나
"정확히 어디였는지" 더 파고들지 않는다. 5번째 응답이 이미 지나갔다면 몸을 소재로 삼더라도 위치가 아니라
감각의 결이나 세기(뜨거웠는지, 조여드는 느낌이었는지 등), 혹은 완전히 다른 화제(행동·충동·타이밍 등)로
향한다. 위치를 반복해서 캐묻는 건 실제 상담사의 화법이 아니라 신체 진단처럼 느껴져서 사용자를 지치게 한다.
...
// 화면 쪽 로직에 영향을 주지 않는다.
const BREATHER_INSTRUCTION_TEMPLATE = (topic: string) => `이 턴은 숨고르기 턴입니다 — 규칙 4의 예외로, 질문을 완전히 금지한다.
...
"더 하실 말씀 있으신가요?", "편하게 적어 주세요"처럼 답을 기다리는 듯한 초대 문장도 쓰지 않습니다 — 반영이
끝나면 그대로 자연스럽게 멈추세요. 이 턴의 마지막 줄이 6·13·17번째 응답 모두 비슷한 형태로 반복되면 안 되니,
그때그때 실제로 나온 이야기에서 나온 표현으로 마무리하세요(정해진 클로징 문구를 재사용하지 말 것).
**응답을 다 쓴 뒤, lines 배열의 모든 문장을 하나씩 훑어 물음표(?)나 "~나요"/"~을까요"/"~인가요"/"~건가요" 같은
의문형 어미로 끝나는 문장이 있는지 확인한다. 하나라도 있으면 그 문장을 통째로 지우거나 평서문으로 다시 쓴다 —
이 턴에서 질문형 문장이 단 하나라도 남아 있으면 이번 응답 전체가 실패다.**`;
...
function buildQuizQuotePreamble(item?: QuizAnswerQuote | null): string {
  if (!item) return "";
  return `**중요, 반드시 지킬 것 — 생략 금지**: 이번 응답은 위 단계 지침이 원래 묻던 질문을 쓰지 않고, 심리테스트 답변을 계기로 삼은 질문 하나로 반드시 대체한다(규칙 4 — 질문은 응답당 1개, 위 지침의 질문과 이 질문을 둘 다 넣지 않는다). 순서:
1. 먼저 사용자가 아까 심리테스트 문항 "${item.prompt}"에서 "${item.label}"라고 답했다는 걸 사용자가 알아볼 수 있게 직접 인용한다 — 예: "아까 '${item.prompt}' 질문에서는 '${item.label}'라고 답해 주셨는데,"처럼(그대로 베끼지 말고 지금 대화 흐름과 언어에 맞게 자연스럽게 새로 쓸 것, 영어 필드 이름은 절대 말하지 않는다).
...
export function buildChatSystemPrompt(
  ...
  const phaseInstruction = quotePreamble ? `${basePhaseInstruction}\n\n${quotePreamble}` : basePhaseInstruction;
  // 규칙 7 하나만 믿지 않고 매 턴 "지금 해야 할 일" 바로 옆에서 다시 상기시킨다 — 긴 규칙 블록 속
  // 문장 하나보다, 지금 응답 직전의 지시가 모델에게 훨씬 더 잘 지켜진다(2026-09-23, 실기기에서 turn 5
  // 이후에도 몸 감각 위치를 반복해서 묻는 게 관찰돼 추가).
  const bodyLocationReminder =
    effectiveTurn > 5
      ? "\n\n(주의: 몸 감각의 위치는 5번째 응답에서 이미 물었다. 이번 응답에서 몸을 소재로 쓰더라도 위치는 다시 묻지 않는다 — 규칙 7 참고.)"
      : "";
