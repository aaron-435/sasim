...
export const ko = {
  ...
  myReports: {
    heading: "내 리포트",
    empty: "아직 만든 리포트가 없어요. 심리테스트와 AI 상담을 마치면 여기에 저장돼요.",
    savedOn: (date: string) => `${date} 저장`,
  },
  home: {
    ...
    todayOpenCta: "열어보기",
    todayRevisitCta: "오늘 운세 자세히 보기",
    todayUnlockCta: "전체 운세 열기",
    ...
    recentQuestionPrefix: "최근 질문 ·",
    featureQuizLabel: "심리테스트",
    featureQuizDescription: "나를 이해하는 첫걸음 · 끝내면 AI 상담과 심층 리포트가 열려요",
    featureCompatLabel: "궁합",
    featureCompatDescription: "그 사람과 나, 어떤 흐름일까요",
    philosophyLine: "사주는 미래를 맞히는 점술이 아니라, 타고난 기질과 지금의 흐름을 읽는 도구예요.",
  ...
  sajuType: {
    ...
    shareButton: "결과 공유하기",
    shareCardTypeLine: (nickname: string, typeName: string) => `${nickname}님의 사주 유형 · ${typeName}`,
    shareCardFooter: "Fatesaid에서 나의 사주 유형 확인하기",
    celebritiesLabel: "이 유형을 가진 유명인",
    celebrityBirthYear: (year: number) => `${year}년생`,
  },

  sajuLearn: {
    pageTitle: "Fatesaid 활용법 · 사주 알아보기",
    usageHeading: "이렇게 활용해보세요",
    usageBody:
    ...
    introHeading: "사주(四柱)란?",
    introBody:
      "사주는 태어난 연·월·일·시 네 기둥에 담긴 기운을 읽는 한국의 전통 방식이에요. 성격과 타고난 성향, 잘 맞는 환경, 관계에서 마찰이 생기기 쉬운 지점, 그리고 시기에 따라 삶의 초점이 어떻게 달라지는지를 함께 봐요.",
    termsHeading: "먼저 알아두면 좋은 용어 세 가지",
    terms: [
      { label: "사주팔자(四柱八字)", body: "태어난 연·월·일·시로 만들어지는 네 기둥. 각 기둥은 천간(하늘의 기운)과 지지(땅의 기운)가 짝을 이뤄요." },
      { label: "일간(日干)", body: "일주(태어난 날)의 천간으로, 사주에서 '나'를 대표하는 기운이에요. 나머지 모든 기둥은 이 일간을 기준으로 해석돼요 — 사주 전체가 돌아오는 중심축이에요." },
      { label: "오행(五行)", body: "목(木)·화(火)·토(土)·금(金)·수(水). 이 다섯 기운이 사주 안에서 어떻게 균형을 이루는지가 기운의 흐름을 설명해줘요." },
    ],
    elementsHeading: "오행이 왜 중요할까요",
    elementsBody:
      "목표는 오행을 완벽하게 고르게 만드는 게 아니에요 — 애초에 그런 사주는 거의 없어요. 중요한 건 어떤 오행이 강하고 어떤 오행이 약한지, 그리고 그 패턴이 무엇을 의미하는지예요. 화(火)가 강한 사주는 표현이 많고 존재감이 드러나는 경향이 있고, 수(水)가 강한 사주는 관찰력이 좋고 유연하게 적응하는 경향이 있어요. 약한 오행은 결함이 아니라, 의식적으로 채워주면 좋은 지점을 알려주는 신호에 가까워요.",
    cyclesHeading: "대운(大運)이 더해주는 것",
    cyclesBody:
      "사주 원국 자체는 고정된 패턴을 보여주지만, 사주는 여기에 '대운'이라는 흐름도 함께 봐요 — 대략 10년 단위로 삶의 초점이 안정, 존재감, 책임, 변화, 관계 중 어디로 향하는지가 바뀌는 시기예요. 그래서 사주는 고정된 성격표라기보다, 걸어가는 동안 계속 바뀌는 지도에 가까워요. Fatesaid의 대운 흐름 기능이 바로 이 개념을 바탕으로 만들어졌어요.",
  },
  ...
  fortune: {
    ...
    lockedBody: "구독하면 아래가 모두 열려요.",
    lockedBenefits: [
      { title: "오늘의 운세", body: "매일 아침 봉인을 열고 오늘의 리듬, 재물·애정·건강, 행운 포인트까지" },
      { title: "이번 주", body: "7일의 흐름과 그중 가장 좋은 날" },
      { title: "이달의 캘린더", body: "이번 달 하루하루의 리듬 — 가장 좋은 날과 페이스 조절이 필요한 날" },
      { title: "신년 운세", body: "다가오는 해와 그 12개월의 흐름, 주제별로" },
      { title: "사주 Q&A 하루 10개", body: "내 사주로 답해 주는 질문을 하루 10개까지" },
    ],
    ...
    scoreLabel: "오늘의 리듬",
    rhythmNames: { mirror: "나다운 리듬", selfNurturesOther: "베푸는 리듬", otherNurturesSelf: "받는 리듬", selfChallengesOther: "이끄는 리듬", otherChallengesSelf: "고르는 리듬" },
    ...
    yearLifeStageLabel: "신년의 12운성",
    yearSinsalLabel: "신년의 12신살",
    monthlyFlowLabel: "월별 흐름",
    hapBadge: "합",
    chungBadge: "전환",
  ...
  report: {
    ...
    paywallBody: "대화 인용 분석, 사주×심리 교차분석, 강점·약점, 맞는 환경, 행동 지침까지 — 이 리포트의 나머지 부분이 마지막 페이지까지 이어집니다.",
    ...
    errorRateLimited: "요청이 많아 잠시 후 다시 시도해주세요.",
    purchaseUnavailable: "지금은 이 리포트를 구매할 수 없어요. 무료 페이지는 그대로 볼 수 있어요.",
