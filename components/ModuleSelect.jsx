  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", background: "#122019", backgroundImage: "radial-gradient(circle at 50% -10%, rgba(111,169,139,0.10), transparent 55%)", display: "flex", justifyContent: "center" }}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .ms-card { width: 100%; text-align: left; background: rgba(255,255,255,0.03); border: 1px solid #26332B; border-radius: 14px; padding: 18px 20px; color: #D9C9A3; cursor: pointer; transition: border-color 0.18s ease, background 0.18s ease; display: flex; align-items: center; justify-content: space-between; gap: 12px; }
        .ms-card:hover { border-color: #6FA98B; background: rgba(111,169,139,0.05); }
      ` }} />
      <div className="ms-root" style={{ width: "100%", maxWidth: "460px", padding: "56px 22px 64px" }}>
        <div style={{ textAlign: "center", marginBottom: "32px" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "6px", fontSize: "11px", letterSpacing: "0.16em", color: "#6FA98B", textTransform: "uppercase", marginBottom: "14px" }}>
            <Sparkles size={12} strokeWidth={1.75} />
            {t.moduleSelect.badge}
          </div>
          <h1 className="ms-serif" style={{ fontSize: "26px", fontWeight: 500, color: "#D9C9A3", margin: 0, lineHeight: 1.4 }}>
          ...
          {MODULES.map((m) => (
            <button key={m.id} type="button" className="ms-card" onClick={() => onSelect?.(m.id)}>
              <div>
                ...
                <div style={{ fontSize: "12.5px", color: "#9C9277" }}>{m.subtitle}</div>
              </div>
              <ArrowRight size={17} strokeWidth={2.25} color="#6FA98B" />
            </button>
          ))}
        </div>
      </div>
